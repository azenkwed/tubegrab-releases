# TubeGrab Releases
Version: v0.3.11

TubeGrab is a desktop app for downloading audio or video from YouTube and YouTube Music.

## Download
- Latest release: https://github.com/azenkwed/tubegrab-releases/releases/latest
- Windows: `TubeGrab-Setup.exe`
- macOS: `TubeGrab-<version>.dmg`
- Linux: `TubeGrab-<version>.AppImage`

## Install
Windows:
- Run the installer and follow the prompts.

macOS:
- Open the DMG and drag TubeGrab into Applications.
- On first launch, you may need to allow the app in System Settings.

Linux:
- Mark the AppImage as executable and run it.

## Help
- User guide (PDF): https://azenkwed.github.io/tubegrab-releases/USER_MANUAL.pdf

## Support
- Donate: https://ko-fi.com/azenkwed
